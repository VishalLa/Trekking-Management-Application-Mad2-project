<template>
  <div>
    ...
    <div class="section-header">
      <h2 class="section-title">Reports & KPIs</h2>
    ...
    <template v-else-if="stats">
      ...
      <div class="panel full-panel">
        ...
        <table class="rev-table">
          ...
          <tbody>
            <tr
              v-for="(t, i) in stats.trek_metrics.top_5_revenue_generators"
              :key="t.id"
            >
...
export default {
  ...
  computed: {
    kpiCards() {
      ...
      return [
        { label: 'Total Revenue',  value: '₹' + o.total_revenue.toLocaleString('en-IN', { maximumFractionDigits: 0 }), sub: 'All paid bookings' },
        { label: 'Total Bookings', value: o.total_bookings, sub: (this.stats.booking_metrics.by_status.BOOKED || 0) + ' active' },
        { label: 'Total Treks',    value: o.total_treks,    sub: (this.stats.trek_metrics.by_status.OPEN   || 0) + ' open now' },
        { label: 'Total Trekkers', value: this.stats.user_metrics.total_trekkers, sub: this.stats.user_metrics.total_staff + ' staff' },
      ]
    },
    trekStatusBars() {
      if (!this.stats) return []
    ...
    bookingItems() {
      if (!this.stats) return []
      const colors = { BOOKED: '#16a34a', CANCELLED: '#dc2626', COMPLETED: '#2563eb' }
...
.primary-btn {
  padding: 8px 16px; background: #1a6b42; border: none; border-radius: 6px;
...
.kpi-card {
  background: #fff; border: 1px solid #dde1e7; border-radius: 8px; padding: 16px 18px;
}
...
.panel {
  background: #fff; border: 1px solid #dde1e7; border-radius: 8px; padding: 18px 20px;
}
