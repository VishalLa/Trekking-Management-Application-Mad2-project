  ...
  <div>
    <div class="section-header">
      ...
      <div class="header-right">
        <SearchBar v-model="query" placeholder="Search treks…" />
        
        <button v-if="role === 'ADMIN'" class="primary-btn" @click="openCreate">
          + Create Trek
        </button>
      </div>
    </div>
        ...
        <div class="list-card">
          ...
          <div class="card-actions">
            ...
            <template v-if="role === 'ADMIN' || role === 'STAFF'">
              ...
              <select class="action-select" :value="trek.status" @change="updateStatus(trek, $event.target.value)">
                <option v-if="role === 'ADMIN'" value="PENDING">Pending</option>
                <option v-if="role === 'ADMIN'" value="APPROVED">Approve</option>
                <option value="OPEN">Open</option>
                <option value="CLOSED">Close</option>
                <option value="COMPLETE">Complete</option>
              </select>

              <button v-if="role === 'STAFF'" class="action-btn btn-outline" @click="askUpdateSlots(trek)">
                Edit Slots
              </button>

              <button v-if="role === 'ADMIN'" class="action-btn btn-bookings" :class="{ active: expandedId === trek.trek_id && activeTab === 'staff' }" @click="togglePanel(trek.trek_id, 'staff')">
                🧑‍🤝‍🧑 Staff
              ...
              <button class="action-btn btn-bookings" :class="{ active: expandedId === trek.trek_id && activeTab === 'bookings' }" @click="togglePanel(trek.trek_id, 'bookings')">
                📅 Bookings
              ...
              <button v-if="role === 'ADMIN'" class="action-btn btn-outline" @click="openUpdate(trek)">Update</button>
              <button v-if="role === 'ADMIN'" class="action-btn btn-danger-outline" @click="askDelete(trek)">Delete</button>
            </template>
...
export default {
  ...
  computed: {
    pageTitle() {
      if (this.role === 'ADMIN') return 'Trek Management'
      if (this.role === 'STAFF') return 'My Assigned Treks'
    ...
    filtered() {
      ...
      if (q) list = list.filter(t =>
        t.trek_name?.toLowerCase().includes(q) || t.location?.toLowerCase().includes(q)
      )
      return list
    }
  },
  ...
  methods: {
    ...
    updateBookingCount(trekId, count) {
      this.bookingCounts = { ...this.bookingCounts, [trekId]: count }
    },

    updateStaffCount(trekId, count) {
      this.staffCounts = { ...this.staffCounts, [trekId]: count }
    },
    ...
    handleSuccessfulBooking(bookingId) {
    ...
    async load() {
      ...
      try {
        ...
        if (this.role === 'ADMIN') {
          endpoint = '/admin/list-trek'
        }
        if (this.role === 'STAFF') { 
          endpoint = `/staff/assigned-trek-list/${this.userId()}`
        }
        if (this.role === 'TREKKER') {
          endpoint = `/trekker/trek-list`
        }
    ...
    async updateStatus(trek, status) {
      const prefix = this.role === 'ADMIN' ? '/admin' : '/staff'
      try {
        const res = await fetch(`${prefix}/trek/${trek.trek_id || trek.id}/${status}`, { 
          method: 'PUT', 
          headers: this.headers() 
        })
    ...
    async confirmDelete() {
      ...
      try {
        await fetch(`/admin/trek/${this.deleteTarget.trek_id}/delete`, {
          method: 'DELETE', 
          headers: this.headers() 
        })
    ...
    async askUpdateSlots(trek) {
        const input = prompt(`Update available slots for ${trek.trek_name}:`, trek.available_slots)
        if (!input) return
        const newSlots = parseInt(input, 10)
        
        try {
            await fetch(`/staff/trek/${trek.trek_id}/slots/${newSlots}`, { 
              method: 'PUT', 
              headers: this.headers() 
            })
